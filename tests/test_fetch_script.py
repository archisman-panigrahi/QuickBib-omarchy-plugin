...
class FetchTest(unittest.TestCase):
    ...
    def test_crossref_request_is_streamed_without_query_changes(self):
        response = mock.MagicMock()
        ...
        with mock.patch.dict(sys.modules, {"doi2bib3.backend": backend}):
            with quickbib_fetch.bounded_doi2bib3_requests():
                requests.get("https://api.crossref.org/works?query.bibliographic=test&rows=5")
        called_url = requests.get.call_args.args[0]
        self.assertIn("rows=5", called_url)
        self.assertNotIn("select=", called_url)
